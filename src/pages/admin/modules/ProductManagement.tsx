...
import { Search, Plus, Edit2, Trash2, Save, X, Eye } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <Modal isOpen={showFormModal} onClose={() => setShowFormModal(false)} title={selectedProduct ? 'Sửa sản phẩm' : 'Thêm sản phẩm'} size="lg">
          ...
          <div className="mt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-gray-700 mb-1">Dung lượng</label>
                <input value={dungLuong} onChange={e => setDungLuong(e.target.value)} className="w-full px-3 py-2 border rounded" />
              </div>
              <div>
                <label className="block text-sm text-gray-700 mb-1">Băng thông</label>
                <input value={bangThong} onChange={e => setBangThong(e.target.value)} className="w-full px-3 py-2 border rounded" />
              </div>
              <div>
                <label className="block text-sm text-gray-700 mb-1">Domains</label>
                <input value={domains} onChange={e => setDomains(e.target.value)} className="w-full px-3 py-2 border rounded" />
              </div>
              <div>
                <label className="block text-sm text-gray-700 mb-1">Databases</label>
                <input value={databases} onChange={e => setDatabases(e.target.value)} className="w-full px-3 py-2 border rounded" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm text-gray-700 mb-1">Core</label>
                <input value={core} onChange={e => setCore(e.target.value)} className="w-full px-3 py-2 border rounded" />
              </div>
              <div>
                <label className="block text-sm text-gray-700 mb-1">RAM</label>
                <input value={ram} onChange={e => setRam(e.target.value)} className="w-full px-3 py-2 border rounded" />
              </div>
              <div>
                <label className="block text-sm text-gray-700 mb-1">Network Mbps</label>
                <input value={networkMbps} onChange={e => setNetworkMbps(e.target.value)} className="w-full px-3 py-2 border rounded" />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-semibold">{selectedProduct.danh_muc || '-'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Giá bán</p>
                <p className="font-semibold">{selectedProduct.gia_ban} / {selectedProduct.don_vi}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Trạng thái</p>
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(selectedProduct.trang_thai)}`}>{selectedProduct.trang_thai}</span>
              </div>
              <div>
                <p className="text-sm text-gray-500">Thứ tự hiển thị</p>
                <p className="font-mono">{selectedProduct.thu_tu_hien_thi || '-'}</p>
              </div>
            </div>
