...
import { Search, Plus, Edit2, Trash2, Save, X, Tag } from 'lucide-react';
...
export default function VoucherManagement() {
  ...
  const handleDelete = (voucher: VoucherRecord) => {
    setSelectedVoucher(voucher);
  ...
  const getDiscountDisplay = (voucher: VoucherRecord) => {
    if (voucher.gia_tri?.includes('%')) {
      return voucher.gia_tri;
    }
              ...
              {filteredVouchers.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-6 py-12 text-center text-gray-500">
                    {searchTerm ? 'Không tìm thấy voucher nào' : 'Chưa có voucher nào. Hãy tạo voucher mới!'}
                  </td>
                </tr>
              ) : (
                filteredVouchers.map((voucher) => (
                  <tr key={voucher.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      ...
                      {voucher.dieu_kien && (
                        <div className="text-xs text-gray-500">{voucher.dieu_kien}</div>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm font-semibold text-green-600">{getDiscountDisplay(voucher)}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {parseFloat(voucher.don_toi_thieu || '0').toLocaleString('vi-VN')}₫
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {voucher.so_luong === '0' ? 'Không giới hạn' : voucher.so_luong}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {voucher.da_dung || '0'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      {getRemainingQty(voucher)}
                    </td>
