...
const CKEditorWrapper = dynamic(
  () => import('@ckeditor/ckeditor5-react').then(mod => {
    ...
    return import('@ckeditor/ckeditor5-build-classic').then(editorMod => {
      ...
      return function CKEditorComponent({ data, onChange }: { data: string; onChange: (event: any, editor: any) => void }) {
        return <CKEditor editor={ClassicEditor as any} data={data} onChange={onChange} />;
      };
    });
  }),
  { ssr: false, loading: () => <div className="border rounded p-4 min-h-[200px] animate-pulse bg-gray-100" /> }
);
    ...
    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          <button onClick={() => setShowEditor(false)} className="px-6 py-3 border border-gray-300 rounded-lg hover:bg-gray-50">
            Quay lại
          </button>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-8">
          <div className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  type="text"
                  value={title}
                  onChange={(e) => { setTitle(e.target.value); if (!editingPost) setSlug(toSlug(e.target.value)); }}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  placeholder="Nhập tiêu đề bài viết"
                />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Slug URL <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  value={slug}
            ...
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">Nội dung chính <span className="text-red-500">*</span></label>
              <CKEditorWrapper data={content} onChange={(_, editor) => setContent(editor.getData())} />
            </div>

            <div className="grid md:grid-cols-3 gap-6">
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Danh mục</label>
                <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
              ...
              <div>
                ...
                <select value={status} onChange={(e) => setStatus(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
            ...
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">Thumbnail (upload file)</label>
            ...
            <div className="border-t pt-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Meta Title (60-70 ký tự)</label>
                  <input type="text" value={seoTitle} onChange={(e) => setSeoTitle(e.target.value)} maxLength={70} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Meta Description (150-160 ký tự)</label>
                  <textarea value={seoDesc} onChange={(e) => setSeoDesc(e.target.value)} maxLength={160} rows={3} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Focus Keyword</label>
                  <input type="text" value={tags} onChange={(e) => setTags(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="tag1, tag2, ..." />
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-4 pt-6 border-t">
              ...
              <button onClick={() => setShowEditor(false)} className="px-8 bg-gray-200 hover:bg-gray-300 text-gray-800 py-3 rounded-lg font-semibold">
                Hủy
              </button>
            </div>
          </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={handleNewPost} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-semibold flex items-center gap-2 shadow-lg transition-all">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          ...
          <select value={categoryFilter} onChange={(e) => { setCategoryFilter(e.target.value); setPage(1); }} className="px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
            <option value="all">Tất cả danh mục</option>
            {categories.map(cat => (<option key={cat.id} value={cat.id}>{cat.name}</option>))}
          </select>
          <select value={perPage} onChange={(e) => { setPerPage(Number(e.target.value)); setPage(1); }} className="px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading && <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500">Đang tải...</td></tr>}
                ...
                <tr key={post.id} className="hover:bg-blue-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleEdit(post)} className="p-2 text-green-600 hover:bg-green-100 rounded-lg transition-colors">
          ...
          <div className="flex items-center justify-between mt-4">
            <div className="text-sm text-gray-600">Tổng: {totalItems} bài | Trang {page}/{totalPages}</div>
            <div className="flex items-center gap-2">
