...
import { listTickets, createTicket, updateTicket, deleteTicket, TicketDepartmentPB, TicketPriorityPB, TicketStatusPB, mapPriorityToLabel } from '@/services/tickets';
...
export default function TicketManagement() {
  ...
  const [createData, setCreateData] = useState<{
    tieu_de: string; tin_nhan: string; bo_phan: TicketDepartmentPB;
    do_uu_tien: TicketPriorityPB; trang_thai: TicketStatusPB;
    khach_hang?: string; don_hang?: string;
  }>({ tieu_de: '', tin_nhan: '', bo_phan: 'technical', do_uu_tien: 'trung_binh', trang_thai: 'cho_tech_rep' });
  ...
  const openChat = (t: any) => { setChatTicket(t); setReplyText(''); setShowChat(true); };

  const sendReply = async () => {
    if (!chatTicket || !replyText.trim()) return;
    try {
      const prev = Array.isArray(chatTicket.phan_hoi_cua_he_thong)
        ? chatTicket.phan_hoi_cua_he_thong
        : (chatTicket.phan_hoi_cua_he_thong ? [{ text: String(chatTicket.phan_hoi_cua_he_thong), at: String(chatTicket.updated || new Date().toISOString()) }] : []);
      ...
      setReplyText(''); setShowChat(false); fetchData();
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold flex items-center gap-2">
          <Plus className="h-5 w-5" />Tạo Ticket
        </button>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {loading && <tr><td colSpan={8} className="px-6 py-8 text-center text-gray-500">Đang tải dữ liệu...</td></tr>}
            {!loading && items.length === 0 && <tr><td colSpan={8} className="px-6 py-8 text-center text-gray-500">Không có dữ liệu</td></tr>}
            {items.map((t) => (
              <tr key={t.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap">
                  <p className="font-semibold text-gray-900 font-mono">{t.id}</p>
                  {t.don_hang && <p className="text-xs text-gray-500">Đơn hàng: {t.don_hang}</p>}
                </td>
                ...
                <td className="px-6 py-4">
                  ...
                  <div className="flex items-center text-xs text-gray-500 mt-1">
                    <MessageSquare className="h-3 w-3 mr-1" />{countMessages(t)} tin nhắn
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${t.bo_phan === 'technical' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}`}>
                    {t.bo_phan === 'technical' ? 'Kỹ thuật' : 'Sale'}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${t.do_uu_tien === 'cao' ? 'bg-red-100 text-red-800' : t.do_uu_tien === 'trung_binh' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>
                    {mapPriorityToLabel(t.do_uu_tien)}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <select value={t.trang_thai} onChange={(e) => onUpdateStatus(t.id, e.target.value as TicketStatusPB)} className={`px-3 py-1 rounded-full text-xs font-semibold border ${t.trang_thai === 'cho_tech_rep' ? 'bg-yellow-100 text-yellow-800 border-yellow-300' : t.trang_thai === 'cho_khach_rep' ? 'bg-purple-100 text-purple-800 border-purple-300' : 'bg-gray-100 text-gray-800 border-gray-300'}`}>
                    <option value="cho_tech_rep">Chờ kỹ thuật</option>
                    <option value="cho_khach_rep">Chờ khách hàng</option>
                    <option value="dong_ticket">Đóng</option>
                  </select>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center text-sm text-gray-600">
                    <Clock className="h-4 w-4 mr-1" />{String(t.updated).split(' ')[0]}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
