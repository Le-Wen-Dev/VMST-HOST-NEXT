...
export async function sendOrderNotificationEmail(data: OrderEmailData): Promise<boolean> {
  ...
  if (!EMAILJS_PUBLIC_KEY) {
    console.error('[EmailJS] Missing Public Key! Please set NEXT_PUBLIC_EMAILJS_PUBLIC_KEY in .env file');
    return false;
  }
...
Vui lòng soạn đơn và liên hệ khách hàng để kích hoạt dịch vụ.
    ...
    const response = await send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, templateParams);
