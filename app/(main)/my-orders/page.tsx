...
function MyOrdersContent() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const sorted = [...res.items].sort((a, b) => {
        const ta = new Date((a as any).ngay_dat_hang || a.updated || 0).getTime();
        const tb = new Date((b as any).ngay_dat_hang || b.updated || 0).getTime();
        return tb - ta;
      });
      setOrders(sorted);
  ...
  const displayOrders: DisplayOrder[] = useMemo(() => {
    return orders.map((o) => {
      ...
      const rawOrderDate = (o as any).ngay_dat_hang || (o as any).updated;
