...
export default function MyServicesPage() {
  ...
  useEffect(() => {
    ...
    listMyOrders({ page: 1, perPage: 50, expand: 'san_pham' })
      .then((res) => {
        const mapped: Service[] = res.items
          ...
          const rawDate = (o as any).ngay_dat_hang || o.updated || '';
          const normalizedDate = typeof rawDate === 'string' ? rawDate.replace(' ', 'T') : rawDate;
          // Parse domain from ghi_chu_noi_bo
          const ghiChu = (o as any).ghi_chu_noi_bo || '';
          let domain = '';
          if (ghiChu.includes('|')) {
          ...
          return {
            ...
            startDate: normalizedDate ? new Date(normalizedDate).toLocaleDateString('vi-VN') : '',
                ...
                <div className="grid md:grid-cols-2 gap-8 mb-8">
                  <div>
                    ...
                    <div className="space-y-4">
                      ...
                      {service.domain && (
                      <div>
                        <label className="block text-sm font-semibold text-gray-600 mb-2">Tên miền</label>
                        <div className="flex gap-2">
                          <input
                            type="text"
                          ...
                          />
                          <button
                            onClick={() => handleCopy(service.domain, `domain-${service.id}`)}
                            className="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"
                          >
                            {copiedField === `domain-${service.id}` ? (
                              <CheckCircle className="h-5 w-5 text-green-600" />
                            ) : (
                              <Copy className="h-5 w-5 text-gray-600" />
                            )}
                      ...
                      <div>
                        <label className="block text-sm font-semibold text-gray-600 mb-2">Mật khẩu host</label>
                        <div className="flex gap-2">
                          <input
                            type="text"
                      ...
                      <div>
                        <button
                          onClick={() => handleCopy(
                            `Tên miền: ${service.domain}\nURL quản lý: ${service.panelUrl}\nTài khoản: ${service.username}\nMật khẩu: ${service.password}`,
                            `all-${service.id}`
                          )}
                          className="w-full px-4 py-2 border-2 border-dashed border-blue-300 rounded-lg text-blue-600 hover:bg-blue-50 font-semibold text-sm transition-colors"
                        >
                          {copiedField === `all-${service.id}` ? '✓ Đã copy tất cả' : 'Copy tất cả thông tin'}
                        </button>
                      </div>
                    </div>
